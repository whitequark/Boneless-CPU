...
\begin{itemize}
  ...
  \item \texttt{\{opA, opB\}}: Concatenate the bits of \texttt{opA} and \texttt{opB}. \texttt{opA} makes the high-order bits of the result and \texttt{opB} makes the low-order bits.
  \item \texttt{\{opB\{opA\}\}}: Construct the result by repeating \texttt{opA} \texttt{opB} times.
\end{itemize}
...
\begin{itemize}
  ...
  \item \texttt{decode\_imm\_al(op)}: Calculate the immediate value of an arithmetic or logical instruction according to the following table. If \texttt{op} is not in the table, behavior is \unpredictable.\linebreak
    \begin{tabular}{|c c|}
    \hline
    \texttt{op} & Result \\
    \hline
    ...
    \hline
    \end{tabular}
  \item \texttt{decode\_imm\_sr(op)}: Calculate the immediate value of a shift or rotate instruction according to the following table. If \texttt{op} is not in the table, behavior is \unpredictable.\linebreak
    \begin{tabular}{|c c|}
    \hline
    \texttt{op} & Result \\
    \hline
